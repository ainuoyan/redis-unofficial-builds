...
"src/redis-cli${suffix}" -h 127.0.0.1 -p "$smoke_port" set rzon-build-test ok >/dev/null
[[ "$("src/redis-cli${suffix}" -h 127.0.0.1 -p "$smoke_port" get rzon-build-test)" == ok ]]
...
archive="$OUTPUT_DIR/Redis-Rzon-${REDIS_VERSION}-${PACKAGE_VARIANT}-${PACKAGE_ARCH}"
