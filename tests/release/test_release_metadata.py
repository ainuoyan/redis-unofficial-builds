    ...
    def test_create_and_validate_current_release_set(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                release_metadata.expected_release_names(VERSION, VARIANT),
                {
                    f"Redis-Rzon-{VERSION}-{VARIANT}-x64.tar.gz",
                    f"Redis-Rzon-{VERSION}-{VARIANT}-x64.tar.gz.sha256",
                    f"Redis-Rzon-{VERSION}-{VARIANT}-arm64.tar.gz",
                    f"Redis-Rzon-{VERSION}-{VARIANT}-arm64.tar.gz.sha256",
                    "SHA256SUMS",
                    "manifest.json",
