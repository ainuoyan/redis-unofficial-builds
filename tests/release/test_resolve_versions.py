...
class ResolveVersionsTests(unittest.TestCase):
    ...
    def complete_assets(self, version: str) -> list[dict[str, str]]:
        names = [
            f"Redis-{version}-linux-glibc2.28-x64.tar.gz",
            f"Redis-{version}-linux-glibc2.28-x64.tar.gz.sha256",
            f"Redis-{version}-linux-glibc2.28-arm64.tar.gz",
            f"Redis-{version}-linux-glibc2.28-arm64.tar.gz.sha256",
            "SHA256SUMS",
            "manifest.json",
