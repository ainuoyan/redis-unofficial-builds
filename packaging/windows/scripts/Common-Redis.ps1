...
$script:RedisPrefix = 'C:\Program Files\Redis-Rzon'
$script:RedisServiceName = 'RedisRzon'
$script:RedisStateFile = Join-Path $script:RedisPrefix '.redis-package-state.json'
$script:RedisBackupRoot = 'C:\ProgramData\Redis-Rzon\Backups'
...
function Enter-RedisLifecycleLock {
    $script:LifecycleMutex = New-Object -TypeName Threading.Mutex `
        -ArgumentList @($false, 'Global\RedisRzonLifecycle')
    try {
...
daemonize no
supervised no
dir "/c/Program Files/Redis-Rzon/data"
logfile "/c/Program Files/Redis-Rzon/log/redis.log"
pidfile "/c/Program Files/Redis-Rzon/run/redis.pid"
'@
...
function New-RedisService {
    if ($null -ne (Get-RedisService)) { throw 'RedisRzon service already exists.' }
    $wrapper = Join-Path $script:RedisPrefix 'bin\RedisService.exe'
    $binaryPath = '"' + $wrapper + '" --service'
    New-Service -Name $script:RedisServiceName -BinaryPathName $binaryPath -DisplayName 'Redis Rzon (experimental)' `
...
function Remove-RedisService {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw 'Unable to delete the RedisRzon service.' }
    } finally {
        $service.Dispose()
    }
    ...
    throw 'RedisRzon service remained marked for deletion.'
}
...
function Start-RedisServiceAndWait {
    ...
    throw 'RedisRzon service did not pass the Redis PING readiness check.'
}
