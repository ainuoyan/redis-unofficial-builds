...
if [[ "$recovering_uninstalled" == false ]]; then
  ...
  install -m 0644 "$REDIS_PLIST" "$backup/com.rzon.redis.plist"
fi
was_running=false
...
rollback() {
  ...
  if [[ -f "$backup/com.rzon.redis.plist" ]]; then
    install -m 0644 "$backup/com.rzon.redis.plist" "$REDIS_PLIST"
  fi
...
install -m 0644 "$package_root/launchd/com.rzon.redis.plist" "$REDIS_PLIST"
