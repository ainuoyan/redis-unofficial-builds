...
if [[ "$recovering_uninstalled" == false ]]; then
  [[ -f "$REDIS_PLIST" && ! -L "$REDIS_PLIST" ]] \
    || die "Managed LaunchDaemon file is missing or unsafe." "受管理的 LaunchDaemon 文件不存在或不安全。"
...
rollback() {
  ...
  if [[ "$was_running" == true ]]; then
    start_service || die "Files restored, but the old service could not start; backup: $backup" "文件已恢复，但旧服务无法启动；备份：$backup"
