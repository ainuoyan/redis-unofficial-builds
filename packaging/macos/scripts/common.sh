...
readonly REDIS_USER="_redis_rzon"
readonly REDIS_GROUP="_redis_rzon"
readonly REDIS_LABEL="com.rzon.redis"
readonly REDIS_DOMAIN_LABEL="system/com.rzon.redis"
readonly REDIS_PLIST="/Library/LaunchDaemons/com.rzon.redis.plist"
readonly REDIS_STATE_FILE="$REDIS_PREFIX/.redis-package-state"
readonly REDIS_SOCKET="$REDIS_PREFIX/data/redis.sock"
readonly REDIS_LOCK_DIR="/var/run/redis-rzon.lifecycle.lock"
...
validate_package() {
  ...
  [[ -x "$root/bin/redis-server" && ! -L "$root/bin/redis-server" \
    ...
    && -f "$root/launchd/com.rzon.redis.plist" && ! -L "$root/launchd/com.rzon.redis.plist" ]] \
  ...
  /usr/bin/plutil -lint "$root/launchd/com.rzon.redis.plist" >/dev/null \
...
ensure_service_account() {
  ...
  if dscl . -read "/Groups/$REDIS_GROUP" >/dev/null 2>&1; then
    ...
    dscl . -create "/Groups/$REDIS_GROUP" RealName "Redis Rzon service"
  ...
  if dscl . -read "/Users/$REDIS_USER" >/dev/null 2>&1; then
    ...
    dscl . -create "/Users/$REDIS_USER" RealName "Redis Rzon service"
...
unixsocketperm 770
pidfile /var/run/redis-rzon.pid
EOF
}
...
install_program_files() {
  ...
  install -m 0644 "$root/launchd/com.rzon.redis.plist" "$REDIS_PREFIX/launchd/"
