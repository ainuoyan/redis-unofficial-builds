...
require_root
require_commands pgrep sleep awk find findmnt flock rc-service rc-update rm stat
acquire_lock
...
if [[ "$purge" == true ]]; then
  ...
  info "Removed Redis program and OpenRC service; conf, data, logs, and state were preserved." "已删除 Redis 程序和 OpenRC 服务；配置、数据、日志和状态已保留。"
fi
