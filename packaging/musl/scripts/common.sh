...
acquire_lock() {
  if [[ -e "$REDIS_LOCK_FILE" && ( ! -f "$REDIS_LOCK_FILE" || -L "$REDIS_LOCK_FILE" ) ]]; then
    die "Lifecycle lock path is unsafe: $REDIS_LOCK_FILE" "生命周期操作锁路径不安全：$REDIS_LOCK_FILE"
  fi
  exec 9>"$REDIS_LOCK_FILE"
  chmod 0600 "$REDIS_LOCK_FILE"
  flock -n 9 || die "Another Redis lifecycle operation is running." "另一个 Redis 生命周期操作正在运行。"
}
...
validate_package() {
  ...
  [[ "$(metadata_value "$root/PACKAGE-INFO" PACKAGE_FORMAT)" == 3 \
    ...
    || die "The package metadata does not match the musl/OpenRC contract." "安装包元数据不符合 musl/OpenRC 约定。"
  package_arch="$(metadata_value "$root/PACKAGE-INFO" PACKAGE_ARCH)"
  machine="$(uname -m)"
  case "$machine:$package_arch" in x86_64:x64|aarch64:arm64) ;; *) die "Package architecture does not match $machine." "安装包架构与 $machine 不匹配。" ;; esac
  ...
  [[ "$uid" =~ ^[1-9][0-9]*$ && "$gid" =~ ^[1-9][0-9]*$ ]] \
    || die "No safe unprivileged account is available for binary validation." "没有可用于验证二进制的安全低权限账号。"
  ...
  if grep -Fq 'not found' <<<"$ldd_output"; then
    die "The Redis binary has unresolved runtime dependencies." "Redis 二进制存在未满足的运行时依赖。"
  fi
}
...
ensure_service_account() {
  if getent group "$REDIS_GROUP" >/dev/null; then
    [[ "$(getent group "$REDIS_GROUP" | awk -F: '{print $3}')" != 0 ]] \
      || die "Refusing to use a GID 0 Redis group." "拒绝使用 GID 为 0 的 Redis 用户组。"
  else
  ...
  if getent passwd "$REDIS_USER" >/dev/null; then
    ...
    [[ "$(awk -F: '{print $3}' <<<"$record")" != 0 \
      ...
      || die "Existing redis account is not a compatible system account." "现有 redis 账号不是兼容的系统账号。"
  else
...
refuse_nested_mounts() {
  ...
  listing="$(findmnt -rn -o TARGET 2>/dev/null)" \
    || die "Unable to inspect the system mount table." "无法检查系统挂载表。"
  for path in "$@"; do
    ...
    [[ -z "$unexpected" ]] \
      || die "Refusing to remove a mount point or a directory containing one: $unexpected" "拒绝删除挂载点或包含挂载点的目录：$unexpected"
  done
}
