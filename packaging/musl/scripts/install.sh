...
if [[ -e "$REDIS_PREFIX" || -L "$REDIS_PREFIX" || -e "$REDIS_INIT_SCRIPT" || -L "$REDIS_INIT_SCRIPT" ]]; then
  die "Refusing to overwrite an existing installation or OpenRC service." "拒绝覆盖现有安装或 OpenRC 服务。"
fi
...
wait_ready "$REDIS_PREFIX" || die "Redis did not pass its OpenRC readiness check." "Redis 未通过 OpenRC 就绪检查。"
trap release_lock EXIT
trap - INT TERM HUP
info "Installed Redis $version as the OpenRC service $REDIS_SERVICE." "已将 Redis $version 安装为 OpenRC 服务 $REDIS_SERVICE。"
