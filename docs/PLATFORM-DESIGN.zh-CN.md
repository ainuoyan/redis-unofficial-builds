...
`--with-lg-page=16`（64 KiB），避免构建主机的 4 KiB 页大小限制运行主机。
构建继续使用固定的用户态基线及全新解压的源码目录，必须重编 Redis 和 jemalloc，
不能通过替换服务脚本修复旧二进制。此参数不改变内核页大小或 THP 设置。
新产物发布前须在 ARM64 4 KiB 和 64 KiB 内核上分别验证读写、RDB/AOF 持久化、
重启和完整服务生命周期，并观察内存占用；仅通过构建参数测试不代表实机验收通过。
通过后使用新的发布修订，不覆盖已有产物。x64、macOS 和 Windows 的设置不变。
